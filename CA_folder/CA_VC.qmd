...
Each row here is a set of parameters the model tried and its metrics. We can fit the best model to our data and see how it performs on the test data set aside at the beginning of this analysis. 
...
Here it is! The final log loss for this knn model is 2.417 while the accuracy is 48.54%.  
...
write.csv(knnsub, "CA_output/knnSub3.csv", row.names = FALSE)
```
...
This final result is an improvement over the knn model in terms of log loss (1.290 compared to 2.417), but actually performs a bit worse in terms of accuracy (43.03% to 48.54%). 
...
rf_rec <- recipe(priceRange ~ ., data = train) |>
  ...
  step_other(homeType, threshold = 0.1) |>
  step_novel(all_nominal_predictors())
...
rf_starts <- data.frame(
  steps = c("base", "novel", "other 0.05", "other 0.10", "log"),
  accuracy = c(0.5526529, 0.5507713,	0.5524935, 0.555008, 	0.5493572),
  log_loss = c(1.0613658, 1.0660283,	1.0664596, 1.061050,	1.0651181)
)
