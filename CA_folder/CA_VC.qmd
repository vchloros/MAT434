...
While using the testing data, the model had an accuracy of 83.32% and a log loss of 0.63. The accuracy was slightly higher compared to the training data but the log loss was slightly higher as well. The similarity of the two is a good sign; if there were an egregious amount of overfitting, training would have inflated metrics compared to the testing data.
...
Here, the test data shows an accuracy of 44.78% and a log loss of 1.28. This is a stark contrast from the metrics of the fitted knn model, which had an accuracy of 81.73% and a log loss of 0.61. In comparison, it looks like the decision tree model is performing worse than the knn.

##### Test Fit

```{r dt test model fitting}
#| code-fold: show
...
For the testing data, the accuracy was 47.03% and the log loss was 1.21. Again, this performance is worse than that of the knn model. 

It seems like, unlike the knn model, the decision tree model experienced limited improvement with additional steps, testing, and tuning. Its initial log loss rating of 1.36 was only improved down to 1.21 while the knn model had the much more impressive dive from 6.81 (with all steps included) down to 0.63. 
...
Once fit, it seems like the random forest model has the best metrics so far. Accuracy is at 97.66% while 
...
write.csv(rfsub, "RFSub1.csv", row.names = FALSE)
```
