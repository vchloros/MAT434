...
Each row here is a set of parameters the model tried and its metrics. We can fit the best model to our data and see how it performed. 
...
Here it is! The final log loss for this knn model is 0.608 while the accuracy is 81.72%. This is a pretty great figure after a lot of testing and retesting, but there are still other models to try. 
...
I used the same steps as I used in the nearest neighbor model. 
...
```
```{r}
dt_step_iterations <- data.frame(
  step_added = c("base", "log", "other", "novel"),
  accuracy = c(),
  log_loss = c()
)
```
