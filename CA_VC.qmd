...
A K-nearest neighbors model, or KNN for short, 
...
In the recipe, I added some steps for our data before running the model. `step_rm` removes variables from being used in the model. I excluded `id` and `description` at this point since they are each specific to one observation. Having them used to predict price range would cause egregious overfitting. I also removed latitude and longitude from the model since I wasn't sure how to process the two. 

From that base recipe, I added some more steps after iterating. I added a step to transform the variable for lot size logarithmically since that aided in its visualization in the EDA. `step_other` was added for both city and homeType variables since they largely comprised one value with a handful of others. This step allows the model to sort the rest of the values into "other" to compare to the majority.

This next step takes our workflow and applies it to training folds created earlier. These folds are equally-sized samples of the data used for cross-validation. Additionally, parallel processing is implemented here to make these functions finish faster by splitting the workload between multiple cores.
...
The results show the accuracy and the log loss of this first iteration of the model. 
...
#Elapsed time in RStudio: 40 secs
...
write.csv(knnsub, "knnSub2.csv", row.names = FALSE)
```
...
write.csv(dtsub, "DTSub2.csv", row.names = FALSE)
```
...
rf_spec <- rand_forest(min_n = tune(), mtry = tune(), trees = tune()) |>
  set_engine("ranger") |>
  set_mode("classification")
