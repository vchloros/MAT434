...
write.csv(knnsub, "knnSubInitial.csv", row.names = FALSE)
```
...
#| eval: false
log_loss <- dt_tune_results |>
  augment(train) |>
  mutate(priceRange = as.factor(priceRange)) |>
  mn_log_loss(priceRange, starts_with(".pred"), - .pred_class)

accuracy_metric <- dt_tune_results |>
  augment(train) |>
  mutate(priceRange = as.factor(priceRange)) |>
  mn_log_loss(priceRange, .pred_class)
...
write.csv(dtsub, "DTSubInitial.csv", row.names = FALSE)
```
...
rf_rec <- recipe(priceRange ~ ., data = train) |>
  ...
  step_dummy(all_nominal_predictors()) |>
  step_impute_median(all_numerical_predictors())
...
rf_tune_results |>
  show_best(metrics = "recall")

best_params <- rf_tune_results |>
  select_best(metrics = "recall")
